...
- **KiCad 导入语义须还原（下单工程为 `SPINC-JLC-Rev-A4`）。** 2026-09-25 实测 JLCEDA 4.1.60 的 KiCad 导入器照抄三类数值而不翻译语义：叠层厚度 mm 原样写入 mil 字段（3D 板厚 0.04 mm）；KiCad 钥匙孔多边形被拆成各自实心的 FILL（KiCad Logo 的镂空印成白块）；KiCad 大写字高被当作 JLC 字号（字高仅约 56%，top/bottom 对齐丢失）；多边形描边宽度一律变成 0.2 mil（pin-1 三角偏小）。flow 阶段 `repair-import-semantics` / `import-semantic-project`（harness PR #7 起的 `jlc pcb epro2-repair-kicad-import`：JLC 字体墨迹高度按五点实测线性模型、竖直对齐按 KiCad 8 `FONT::getLinePositions`、多行按 KiCad 行距拆行；多边形描边按闭合 POLY 还原）以冻结 `.kicad_pcb` 为真值还原这三项并导入为新工程 `SPINC-JLC-Rev-A4`；此后 DRC、导出与闸门都在 A4 上执行。中间工程均已被取代、不可下单（其上做过标定预演，删除需用户本人操作）：`SPINC-JLC-Rev-A2`（harness `f6d03bd`：单点字号标定、BOTTOM 语义未换算、多边形描边未恢复）、`SPINC-JLC-Rev-A3`（harness `f562692`：描边误写入不渲染的 FILL width）。`gerber-equivalence` 自此把丝印纳入硬闸门（非文字几何零容差级、文字按大写字高与位置、两侧无无法解释的墨迹），`stackup-3d`（`JLC/verify_3d_stackup.py`）核对叠层与 3D 模型（板厚、挖槽为孔）。
