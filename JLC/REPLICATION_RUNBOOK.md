...
- **KiCad 导入语义须还原（下单工程为 `SPINC-JLC-Rev-A6`）。** 2026-09-25 实测 JLCEDA 4.1.60 的 KiCad 导入器照抄三类数值而不翻译语义：叠层厚度 mm 原样写入 mil 字段（3D 板厚 0.04 mm）；KiCad 钥匙孔多边形被拆成各自实心的 FILL（KiCad Logo 的镂空印成白块）；KiCad 大写字高被当作 JLC 字号（字高仅约 56%，top/bottom 对齐丢失）；多边形描边宽度一律变成 0.2 mil（pin-1 三角偏小）。flow 阶段 `repair-import-semantics` / `import-semantic-project`（harness PR #7 起的 `jlc pcb epro2-repair-kicad-import`：JLC 字体墨迹高度按五点实测线性模型、竖直对齐按 KiCad 8 `FONT::getLinePositions`、多行按 KiCad 行距拆行；多边形描边按闭合 POLY 还原）以冻结 `.kicad_pcb` 为真值还原这三项并导入为新工程 `SPINC-JLC-Rev-A6`；此后 DRC、导出与闸门都在 A6 上执行。中间工程均已被取代、不可下单（其上做过标定预演，删除需用户本人操作）：`SPINC-JLC-Rev-A2`（harness `f6d03bd`：单点字号标定、BOTTOM 语义未换算、多边形描边未恢复）、`SPINC-JLC-Rev-A3`（harness `f562692`：描边误写入不渲染的 FILL width）、`SPINC-JLC-Rev-A4`（harness `af6e757`：JLC 默认字体，字高正确但字宽宽 9–24%）、`SPINC-JLC-Rev-A5`（harness `1609274`：DRC 仍有 2 条残留），以及临时工程 `SPINC-DRC-Probe-1`/`-2`。`gerber-equivalence` 自此把丝印纳入硬闸门（非文字几何零容差级、文字按大写字高与位置、两侧无无法解释的墨迹），`stackup-3d`（`JLC/verify_3d_stackup.py`）核对叠层与 3D 模型（板厚、挖槽为孔）。
- **丝印使用 KiCad 原版字形（需本机安装字体）。** JLCEDA 不能导入字体文件，只能使用运行网页编辑器那台机器上已安装的字体（`SYS_FontManager.addFont` 只收字体名），缺字体会静默回退。KiCad 的 newstroke 笔画字体没有官方 TTF，`JLC/fonts/build_kicad_newstroke_ttf.py` 从锁定版本的 KiCad `newstroke_font.cpp`（SHA-256 校验）确定性生成 `KiCad Newstroke T150`/`T200`（线宽/字号 0.15、0.20）；生成物是 GPL-2.0-or-later 衍生作品，不提交，只提交生成器。首次在执行 flow 的 Mac 上运行 `uv run JLC/fonts/build_kicad_newstroke_ttf.py --ratio 0.15 --ratio 0.2 --out ~/Library/Fonts` 安装；preflight `silk-fonts` 以 `--check` 逐字节核对，`import-semantic-project` 在 JLCEDA 页面内再探测一次，丝印闸门按墨迹框四边 ≤0.05 mm 最终裁决。
- **DRC 必须为 0 条。** 此前的两条"已评审残留"都已在根因处消除：网表不一致来自 KiCad `on_board/in_bom` 标志丢失（`--schematic` 还原）、`board_only` Logo 被当成元件（展开为板级图形）、PCB 保留 KiCad 网络名且 56 个元件值被合并器件覆盖（`sync-schematic` 阶段按白名单审计后同步：只允许一对一网络改名、与 KiCad 一致的值/BOM 标志和导入器元数据，移除元件等任何其它变化即取消）；FID2→H1 的 1.3 mil 是 JLCEDA 以外接矩形量器件到焊盘，KiCad 设计无此规则（`pth/npth_inside_courtyard = ignore`），按 `design-invariants.yaml` 的 `drc_other_clearance.device_to_pad_mm: 0` 翻译。`JLC/rev-a/drc-residuals.json` 为空列表；旧的两条保留在 `JLC/fixtures/` 供测试。
